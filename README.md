...
Legacy titles created by older Smart Naming builds are migrated out of tmux window names when their ownership signature can be proven: normal tmux automatic naming is restored, while a still-matching title is retained only as Codex Mux pane metadata. If app-server startup, protocol compatibility, model generation, or validation fails, existing panes and names continue working; failed refreshes are rate-limited and an unhealthy provider is restarted after its cooldown while the current/fallback entry title remains unchanged.
